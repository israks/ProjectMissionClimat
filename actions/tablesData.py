...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        tabControl.add(tab4, text='Photovoltaiques')
        tabControl.add(tab5, text='Chauffages')
        tabControl.add(tab6, text='Isolations')
        tabControl.add(tab7, text='Travaux')
        tabControl.add(tab8, text='Communes')
        ...
        # Travaux
        columns = ('id_travaux', 'cout_total_ht_travaux', 'cout_induit_ht_travaux', 'annee_travaux', 'type_logement_travaux', 'annee_construction_logement_travaux')
        query = """
            SELECT id_travaux, cout_total_ht_travaux, cout_induit_ht_travaux, annee_travaux, type_logement_travaux, annee_construction_logement_travaux
            FROM Travaux
            ORDER BY id_travaux
        """
        ...
        # Communes
        columns = ('code_commune', 'nom_commune', 'statut_commune', 'altitude_moyenne_commune', 'population_commune', 'superficie_commune', 'code_canton_commune', 'code_arrondissement_commune', 'code_departement')
        query = """
            SELECT code_commune, nom_commune, statut_commune, altitude_moyenne_commune, population_commune, superficie_commune, code_canton_commune, code_arrondissement_commune, code_departement
            FROM Communes
